...
in
{
  ...
  options.features.zsh = {
    enable = lib.mkEnableOption "Whether to enable zsh and configure it's defaults";

    completion = lib.mkEnableOption "Whether to enable completion" // {
      default = true;
    };
    historyFile = lib.mkOption {
      type = lib.types.str;
      default = "$HOME/.history";
      description = "Location of zsh's history file";
    };
    maxHistory = lib.mkOption {
      type = lib.types.int;
      default = 2000;
      description = "Maximum history to retain under zsh";
    };
    options = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = [ ];
      description = "List of zsh options to \"set\"";
    };
    substringSearch = lib.mkEnableOption "Whether to enable substring search";
    suggestions = lib.mkEnableOption "Whether to enable auto suggestions" // {
      default = true;
    };
    syntaxHighlight = lib.mkEnableOption "Whether to enable syntax highlighting" // {
      default = true;
    };
  };
